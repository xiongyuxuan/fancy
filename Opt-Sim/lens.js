...
$(function(){
	...
	var layer = new Kinetic.Layer();
		...
		$("#lensgroup").on("mouseover", function(){
			mouseToText.setText("拖动镜片至试验区");
			layer.drawScene();
		});
		$("#objectgroup").on("mouseover", function(){
			mouseToText.setText("请点击一个物体");
			layer.drawScene();
		});
		$("#reset").on("mouseover", function(){
			mouseToText.setText("点击重置可以重置实验");
			layer.drawScene();
		});
		$("#help1").on("mouseover", function(){
			mouseToText.setText("点击帮助按钮可以获得帮助");
			layer.drawScene();
		});
	...
	function drawImage()
	{
		...
		if((lense_1_name!=null || lense_2_name!=null) && (object_name=="arrow" || object_name=="triangle" || object_name=="square")){
			...
			image.on("mouseover", function(){
				mouseToText.setText("双击移除镜片");
				layer.drawScene();
			});
			square_object.on("mouseover", function(){
				mouseToText.setText("请拖动多边形以获得光路图");
				layer.drawScene();
			});
			triangle_object.on("mouseover", function(){
				mouseToText.setText("水平拖动三角物体以获得光路图");
				layer.drawScene();
			});
			arrow_object.on("mouseover", function(){
				mouseToText.setText("水平拖动此箭头以获得光路图");
				layer.drawScene();
			});
